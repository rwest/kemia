...
kemia.io.smiles.parse = function (smi) {
	...
	var bond_stereo = kemia.io.smiles.BondStereo.NONE;
	...
	for (var i = 0; i < items.length; i++) {
		...
		if (item == this.punctuation.nobond) {
			...
			if (ring_atom) {
				...
				bond_stereo = kemia.io.smiles.BondStereo.NONE;
				ring[ringid] = null;
			} else {
...
kemia.io.smiles.sanityCheck = function (branch, ring, bond_type, bond_stereo) {
	if (branch.length) {
...
kemia.io.smiles.parseAtom = function (item) {
  var atom = {isotope:null, symbol:null, stereo:kemia.io.smiles.BondStereo.NONE, hcount:null, charge:null, aromatic:false};
  ...
  if (atomProp) {
    ...
    if (atomProp[3] == kemia.io.smiles.BondStereo.CLOCKWISE) {
	    atom.stereo = atomProp[3];
	} else if (atomProp[3] == kemia.io.smiles.BondStereo.COUNTER_CLOCKWISE) {
	    atom.stereo = atomProp[3]; 
	} else {
 ...
 * @param{kemia.io.smiles.BondStereo}stereo stereo-type code
 ...
 */
kemia.io.smiles.createBond = function(type, stereo, source, target) {
		var atype = type;
